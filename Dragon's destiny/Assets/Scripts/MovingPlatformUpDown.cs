...
{
    //делать по два триггера сверху и снизу (итого четыре)
   ...
   */
    ...
    private void Move()
    {
        Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position + transform.up * direction.x * 0.3F, 0.7F);
        //      Collider2D collider;
